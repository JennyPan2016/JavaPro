...
public class DeliverListDAO {
	...
	//根据groupId查询小组所有发布单，pan.jing
	public List<DeliverList> queryDeliverListByGroupId(int groupId){  
		PreparedStatement pstmt = null;
		Connection conn = new ConnectionDB().connectionDB();
		ResultSet rs = null;
		List<DeliverList> list = new ArrayList<DeliverList>();
		try {
			String sql = "select dl.id, dl.projectId,dl.taskId,dl.title,dl.deliverNo,dl.developer,dl.tester,dl.applicationName,dl.content,dl.comments,dl.createUserId,dl.updateUserId,dl.createTime,dl.updateTime"
					+ " from ProcessPlatform..Department dp , ProcessPlatform..Center c,  ProcessPlatform..PP_Group g, ProcessPlatform..DeliverList dl "
					+ "where dp.id = c.departmentId and c.id = g.centerId and g.id = ?";				
			pstmt = conn.prepareStatement(sql);
			...
			while (rs.next()){
				...
				dList.setUdpateTime(rs.getString("updateTime"));
				list.add(dList);				
			}			
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public DeliverList queryDeliverById(int deliverId){  
		PreparedStatement pstmt = null;
		Connection conn = new ConnectionDB().connectionDB();
		ResultSet rs = null;
		DeliverList dList = new DeliverList();
	...
	public static void main(String[] args) {
		...
		List<DeliverList> list = dListDAO.queryDeliverListByGroupId(1);
		DeliverList dList = new DeliverList();		
		for (int i = 0; i < list.size(); i++) {
			dList = list.get(i);  
			System.out.println(dList.getApplicationName());
		}		
	}
